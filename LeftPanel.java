...
public class LeftPanel extends JPanel implements MyCardPanel {
	private Game game;
	private JPanel cardPanel;
	private PokerGUI gui;
	private OverlapLayout ov;

	public LeftPanel(Game g, PokerGUI pp) {
		setOpaque(false);
		game = g;
		gui = pp;
		...
		ov.setPopupInsets(new Insets(0, 0, 0, 20));
		for (int i = 0; i < p.getNumCards(); i++) {
			JLabel card = new JLabel();
			URL url = LeftPanel.class.getResource("/resources/b1fv.png");
